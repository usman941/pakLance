...
const JobProviderFixedPrice = () => {
    ...
    return async () => {
      try {
        const response = await axios.get(
          `${BackendURI}/provider/getProjectById/${Location.state.projectId}`,
          {
            headers: {
        ...
        console.log("Specific Project", response);
  ...
  const handleSubmit = async (event) => {
    ...
    if (RegisteredRole === "jobProvider") {
      url = `${BackendURI}/provider/updateProject/${Location.state.projectId}`;
      ...
      >
        <div className="w-full max-w-4xl">
          <div className="flex   py-4">
            <p className="text-3xl font-bold">Update a fixed project</p>
          </div>
          <form
